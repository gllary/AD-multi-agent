...
def validate_coordinator_proposal(
    ...
    fixed_next_stage, fixed_action = fixed_threshold_route(
        current_stage,
        risk_score,
        c_thrs,
        a_thrs,
    ...
    if coordinator_action not in CLINICAL_ACTIONS:
        return SafetyDecision(
            ...
            final_action=fixed_action,
            final_next_stage=fixed_next_stage,
            override_reason="invalid_or_missing_action_reverted_to_fixed_threshold",
            safety_risk_level=_risk_to_safety_level(rl),
            policy_basis=["invalid_action", f"risk_level={rl}", "fixed_threshold_pathway"],
            fixed_threshold_action=fixed_action,
            fixed_threshold_next_stage=fixed_next_stage,
        )
    ...
    if coordinator_action not in STAGE_ALLOWED_ACTIONS[current_stage]:
        return SafetyDecision(
            ...
            final_action=fixed_action,
            final_next_stage=fixed_next_stage,
            override_reason="stage_illegal_action_reverted_to_fixed_threshold",
            safety_risk_level=_risk_to_safety_level(rl),
            policy_basis=["illegal_transition", f"risk_level={rl}", "fixed_threshold_pathway"],
            fixed_threshold_action=fixed_action,
            fixed_threshold_next_stage=fixed_next_stage,
        )
    ...
    if rl == "high" and coordinator_action in {"observe_or_reassess", "call_lab_agent", "call_ecg_agent"}:
        return SafetyDecision(
            ...
            policy_basis=policy_basis + ["high_risk_guardrail", "fixed_threshold_pathway"],
            fixed_threshold_action=fixed_action,
            fixed_threshold_next_stage=fixed_next_stage,
        )

    # Intermediate-risk states remain in the pathway when the frozen
    # fixed-threshold policy requires additional evidence.
    if (
        current_stage in {"CP1", "CP2", "CP3"}
        and rl == "intermediate"
        and coordinator_action == "observe_or_reassess"
        and fixed_action != "observe_or_reassess"
    ):
        return SafetyDecision(
            current_stage=current_stage,
            coordinator_action=coordinator_action,
            ...
            final_action=fixed_action,
            final_next_stage=fixed_next_stage,
            override_reason="intermediate_risk_reassessment_reverted_to_fixed_threshold_continuation",
            safety_risk_level=_risk_to_safety_level(rl),
            policy_basis=policy_basis + ["intermediate_risk_continuation", "fixed_threshold_pathway"],
            fixed_threshold_action=fixed_action,
            fixed_threshold_next_stage=fixed_next_stage,
        )
    ...
    if current_stage == "CP4" and rl != "low" and coordinator_action == "observe_or_reassess":
        return SafetyDecision(
            ...
            safety_risk_level=_risk_to_safety_level(rl),
            policy_basis=policy_basis + ["terminal_stage_guardrail", "fixed_threshold_pathway"],
            fixed_threshold_action=fixed_action,
            fixed_threshold_next_stage=fixed_next_stage,
        )
